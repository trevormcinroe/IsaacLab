...
from .abstract_dots import DotsBehaviour, Limits
...
class LinearDotsSource(DotsBehaviour):
    ...
    def update_state(self, state: dict) -> dict:
        def compute_acceleration(positions: np.array, sizes: np.array, gravitation: int):
            ...
            for i in range(len(positions)):
                ...
                force_vectors = relative_positions * gravitation * (sizes**2) / (distances**2)
                accelerations[i] = 0.00001 * np.sum(force_vectors, axis=0)

            return accelerations

        accelerations = compute_acceleration(state["positions"], state["sizes"], self.gravitation)
        ...
        new_state = self._limit_positions(new_state, x_lim=state["x_lim"], y_lim=state["y_lim"])
        return new_state
