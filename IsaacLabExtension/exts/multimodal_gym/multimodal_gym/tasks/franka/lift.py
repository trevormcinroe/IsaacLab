...
class LiftEnvCfg(DirectRLEnvCfg):
    ...
    tiled_camera: TiledCameraCfg = TiledCameraCfg(
        ...
        offset=TiledCameraCfg.OffsetCfg(pos=(0.0, 1.2, 0.2), rot=(1,0,0,0), convention="world"),
        data_types=["rgb"],
...
class LiftEnv(DirectRLEnv):
    ...
    def _get_images(self):
        if not self.init:
            ...
            print(f'OUTPUT: {self._tiled_camera.set_world_poses_from_view(eyes=eyes, targets=targets)}')
            print(f'cam: {self._tiled_camera._view}')
            self.init = True
