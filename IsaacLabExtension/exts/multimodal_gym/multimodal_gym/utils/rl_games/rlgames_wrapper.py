...
from omni.isaac.lab.envs import ManagerBasedRLEnv, VecEnvObs
...
class RlGamesVecEnvWrapper(IVecEnv):
    ...
    def __init__(self, env: ManagerBasedRLEnv, rl_device: str, clip_obs: float, clip_actions: float):
        ...
        if not isinstance(env.unwrapped, ManagerBasedRLEnv) and not isinstance(env.unwrapped, DirectRLElleEnv):
    ...
    def state_space(self) -> gym.spaces.Box | None:
        """Returns the :attr:`Env` :attr:`observation_space`."""
        # note: rl-games only wants single observation space
        critic_obs_space = None  # self.unwrapped.single_observation_space.get("critic")
    ...
    def _process_obs(self, obs_dict: VecEnvObs) -> torch.Tensor | dict[str, torch.Tensor]:
        ...
        if isinstance(obs, LazyFrames):
            # Concatenate along the last dimension from list of torch.Size([128, 80, 80, 3])
            frames = obs._frames
            # now torch.Size([128, 80, 80, 12])
            obs = torch.cat(frames, dim=-1)
            # viz
            # name = "frame_stack_5"
            # file_path = os.path.join("/workspace/isaaclab/IsaacLabExtension/images", f"{name}.png")
            # save_stacked_image(obs, file_path)
