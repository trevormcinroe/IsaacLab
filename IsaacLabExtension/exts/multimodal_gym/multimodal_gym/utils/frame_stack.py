...
class FrameStack(gym.ObservationWrapper, gym.utils.RecordConstructorArgs):
    ...
    def __init__(
        ...
        # self.observation_space = self.observation_space["policy"]

        # low = np.repeat(self.observation_space["policy"].low[np.newaxis, ...], num_stack, axis=0)
        # high = np.repeat(
        #     self.observation_space["policy"].high[np.newaxis, ...], num_stack, axis=0
        # )
        # self.observation_space["policy"] = Box(
        #     low=low, high=high, dtype=self.observation_space["policy"].dtype
        # )
